...
it('combines volunteered answers into tentative, traceable patterns',()=>{
 ...
 expect(cards.find(c=>c.id==='rhythm')?.reading).toContain('meaningful contact without constant contact');
 expect(cards.find(c=>c.id==='qualities')?.reading).toContain('dependable follow-through');
...
it('does not infer diagnoses, reliability or personality from sensitive/custom text',()=>{
 ...
 expect(cards.flatMap(c=>c.evidence)).toContain('Private custom words');
