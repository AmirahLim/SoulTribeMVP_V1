...
export const emptyDraft = ():BaselineDraft => ({...legacyEmpty(),flowVersion:3,setupRevision:1,ageBand:'',ageOther:'',country:'',travelKm:10,intentOther:'',clicksOther:'',qualityOther:'',outingOther:'',connectionChoice:'',connectionOther:'',planningChoice:'',planningOther:'',punctualityChoice:'',punctualityOther:''});
...
export function isDraft(value:unknown):value is BaselineDraft {
 ...
 const shape={...d,...(d.setupRevision===1?{area:'',travel:''}:{}),step:Math.min(d.step,6),desiredQualities:[],outings:[],intent:d.intent.filter(x=>x!=='Other'),clicks:d.clicks.filter(x=>x!=='Other')};
...
export function validStep(d:BaselineDraft,step:number):boolean {
 ...
 return d.setupRevision===1 ? /^[a-z0-9_]{3,20}$/.test(d.handle)&&validIdentity(d,true) : legacyValid(d,6);
}
