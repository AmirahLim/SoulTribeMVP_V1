...
import {privateAvatarUrl} from '../../lib/privateAvatar';
...
export default function ProfilePhoto({userId}:{userId:string}) {
 ...
 useEffect(()=>{void pendingPhoto().then(setPending).catch(()=>setError('Please choose your photo again.'));},[]);
 useEffect(()=>{if(!pending){setPendingUrl('');return;}const u=URL.createObjectURL(pending);setPendingUrl(u);return()=>URL.revokeObjectURL(u);},[pending]);
 async function upload(file:File|undefined) {
  ...
  try {
   ...
   const photoUrl=privateAvatarUrl(path);
   const saved=await client.from('profiles').update({avatar_url:photoUrl}).eq('id',userId).select('id').single();
   if(saved.error||!saved.data)throw new Error('Photo uploaded, but it could not be added to your profile. Please retry.');
   setUserProfile({avatarUrl:photoUrl});
   setUrl(photoUrl);
   try {await pendingPhoto(null);setPending(null);}catch {setError('Photo saved. The device preview could not be cleared.');}
 ...
 return <section className="ob-fields" aria-labelledby="profile-photo-title">
  ...
  {!url&&pendingUrl&&<><img src={pendingUrl} alt="Photo selected during onboarding" width={96} height={96} style={{borderRadius:'50%',objectFit:'cover'}} /><button type="button" disabled={busy} onClick={()=>pending&&void upload(new File([pending],'profile.jpg',{type:'image/jpeg'}))}>Save selected photo to my profile</button></>}
