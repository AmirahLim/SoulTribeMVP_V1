begin;
-- SECURITY INVOKER: RLS remains effective for every operation in this transaction.
create or replace function save_profile_bundle(p_profile jsonb, p_answers jsonb, p_traits jsonb, p_interests integer[] default null)
...
begin
 ...
 update profile_answers set
  onboarding=case when p_answers ? 'onboarding' then p_answers->'onboarding' else onboarding end,
...
revoke all on function save_profile_bundle(jsonb,jsonb,jsonb,integer[]) from public;
grant execute on function save_profile_bundle(jsonb,jsonb,jsonb,integer[]) to authenticated;
commit;
