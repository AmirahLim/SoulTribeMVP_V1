begin;
-- Keep the previous validator available for historical saved drafts.
create or replace function validate_baseline_draft_pre_identity(p jsonb, complete boolean default false)
...
alter table trait_geography add column if not exists country text;
alter table trait_geography add column if not exists radius_km integer check (radius_km between 1 and 50);
...
do $legacy$ begin
 if to_regprocedure('public.claim_onboarding_draft_pre_identity(text,text,integer)') is null then
  execute replace(pg_get_functiondef('public.claim_onboarding_draft(text,text,integer)'::regprocedure),'FUNCTION public.claim_onboarding_draft(','FUNCTION public.claim_onboarding_draft_pre_identity(');
 end if;
end $legacy$;
revoke all on function public.claim_onboarding_draft_pre_identity(text,text,integer) from public,anon,authenticated;
create or replace function claim_onboarding_draft(p_token text,p_display_name text,p_birth_year integer)
returns jsonb language plpgsql security definer set search_path=public as $$
declare d onboarding_drafts; p jsonb; actor uuid:=auth.uid(); contact_styles text[]; interest_ids integer[]; old_answers jsonb;
begin
 ...
 if not(d.payload ? 'setupRevision') then return claim_onboarding_draft_pre_identity(p_token,p_display_name,p_birth_year); end if;
 ...
 perform save_profile_bundle(
  ...
  jsonb_build_object('trait_intent',jsonb_build_object('intents',(p->'intent')-'Other'),
   ...
   'trait_geography',jsonb_build_object('home_area',p->>'area','country',p->>'country','radius_km',(p->>'travelKm')::int,'radius_minutes','{}'::jsonb)),null);
