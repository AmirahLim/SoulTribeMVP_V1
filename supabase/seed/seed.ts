...
export const SYNTHETIC_INTEREST_TREE = [
...
export const SYNTHETIC_PROFILES: ProfileVector[] = Array.from({ length: 40 }, (_, idx) => {
  const num = idx + 1;
  const id = `00000000-0000-0000-0000-${num.toString().padStart(12, '0')}`;
  
  const areas = ['Tiong Bahru', 'Tanjong Pagar', 'Bishan', 'Katong', 'Jurong East', 'Orchard', 'Novena', 'Serangoon', 'Bedok', 'Queenstown'];
  const names = [
  ...
  return {
    profile: {
      ...
      age_pref_max: 50,
      profile_version: 1,
      confidence: idx === 39 ? 0.45 : 0.65 + ((idx % 30) / 100), // profile 40 is low confidence <0.55 for testing
      tier: 'free',
      status: 'active',
    },
    personality: {
    ...
    interests: [
      { user_id: id, node_id: (idx % 5) + 1, node_path: SYNTHETIC_INTEREST_TREE[(idx % 5) + 1].path, node_name: SYNTHETIC_INTEREST_TREE[(idx % 5) + 1].name, affinity: idx % 2 === 0 ? 'regular' : 'curious' },
    ],
    lifestyle: {
      user_id: id,
